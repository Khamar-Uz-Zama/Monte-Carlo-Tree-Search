...
class Board(object):
    ...
    '''
    status_IS_CHECK = 1
    status_IS_CHECKMATE = 2
    status_IS_GAMEOVER = 0
    ...
    def checkStatus(self):
        ...
        if(Board.is_checkmate(self)):
            return Board.status_IS_CHECKMATE, self.turn 
        elif(Board.is_game_over(self)):
            return Board.status_IS_GAMEOVER, self.turn
        elif(Board.is_check(self)):
            return Board.status_IS_CHECK, self.turn        
        else:
            return Board.status_IN_PROGRESS, -1
        
    
    def __init__(self, sfen=None, move_stack=None):
        self.pseudo_legal_moves = PseudoLegalMoveGenerator(self)
        self.legal_moves = LegalMoveGenerator(self)
        Board.ID += 1

        if sfen is None and move_stack is None:
            self.reset()
        elif sfen is None and move_stack is not None:
            # Authot KUZ
            self.reset()
            for i in range(move_stack.__len__()):
                self.push(move_stack[i])
        else:
            self.set_sfen(sfen)
    ...
    def reset(self):
        ...
        self.turn = BLACK
#        print("In RESET, Black == {}".format(BLACK))
#        print("In RESET, White == {}".format(WHITE))
        self.move_number = 1
    ...
    def is_game_over(self):
        ...
        try:
            ...
            print("Game is Stalemate or checkmate by")
            return True

        # Fourfold repetition.
        if self.is_fourfold_repetition():
            print("Game is over due to Four")
            return True
